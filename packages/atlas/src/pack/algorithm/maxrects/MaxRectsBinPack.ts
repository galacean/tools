...
import type { IMaxRectsBinPack } from "@galacean/tools-atlas-algorithm";
...
export class MaxRectsBinPack implements IMaxRectsBinPack {
  ...
  _scoreRectangle(width: number, height: number, method: MaxRectsMethod, score1: Score, score2: Score) {
    let newNode = new Rect();
  ...
  _findPositionForNewNodeBottomLeft(width: number, height: number, bestY: { value: any }, bestX: { value: any }) {
  ...
  _findPositionForNewNodeBestAreaFit(width: number, height: number, bestAreaFit: Score, bestShortSideFit: Score) {
    const freeRectangles = this.freeRectangles;
    const bestNode = new Rect();
  ...
  _commonIntervalLength(i1start: number, i1end: number, i2start: number, i2end: number) {
  ...
  _findPositionForNewNodeContactPoint(width: number, height: number, bestContactScore: Score) {
    const freeRectangles = this.freeRectangles;
    const bestNode = new Rect();
